...
| # | Item | Statut |
|---|------|--------|
| **1** | Fondation Firebase : config + Auth Google + modèle de données Firestore + script de seed + règles de sécurité + récap des données | ⏳ à faire |
...
_(rempli/affiné pendant l'item 1)_

- `users/{uid}` : `{ name, email, picture, appVersion, lastSeenAt }`
- `activities/{activityId}` : tout le détail de l'activité + `createdBy`
- `votes/{uid}/activities/{activityId}` (ou `activities/{id}/votes/{uid}`) : `{ verdict, quotaHit?, updatedAt }`
