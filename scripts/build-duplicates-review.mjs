// Generates `activites-doublons.html` at the repo root — a standalone, offline
// review tool to merge/split look-alike activities and shrink the 198-item list.
//
...
// a self-contained HTML page. The page persists decisions to localStorage and
// can export a JSON verdict (groups + chosen representative + ids to drop).
...
// Full activity payload for the page (so the export can rebuild complete objects).
// Only `photo` is added: app-absolute paths (/photos/…) → repo-relative (public/photos/…)
// so the HTML works when opened directly via file:// from the repo root.
const slim = activities.map((x) => ({
  ...x,
  photo: ((photos[x.id] && photos[x.id][0]) || '/photos/hero.jpg').replace(/^\//, 'public/'),
}));
...
<header>
  ...
  <div class="bar">
    ...
    <button class="btn-primary" id="btn-export-state">Exporter</button>
...
<dialog id="export-state-dlg">
  <div class="dlg-head">Exporter le tri</div>
  <div class="dlg-body">
    <div id="export-alert"></div>
    <p class="hint">Un seul fichier, en 2 parties :<br>
      • <b>activites</b> = ton tri (catégories, décisions, écartés) + le résultat final → sert au travail final <i>et</i> à reprendre le tri.<br>
      • <b>presentation</b> = l'affichage du site (tri du panneau, filtres) → temporaire, juste pour que la personne voie pareil.<br>
      Envoie-le : elle l'ouvre via <b>« Importer »</b> et retrouve exactement ta vue.</p>
    <textarea id="share-text" readonly style="height:230px"></textarea>
  </div>
  <div class="dlg-foot">
    <button id="share-copy">Copier</button>
    <button id="share-dl">Télécharger .json</button>
    <button class="btn-primary" id="share-close">Fermer</button>
  </div>
</dialog>
<dialog id="import-dlg">
  <div class="dlg-head">Importer un tri</div>
  <div class="dlg-body">
    <p class="hint">Choisis un fichier <b>.json</b> reçu, ou colle son contenu. ⚠️ Charger <b>remplace</b> ton tri actuel — mais une sauvegarde est faite et tu pourras l'annuler avec <b>« ↩ Annuler »</b>.</p>
    <input id="import-file" type="file" accept="application/json,.json" style="margin-bottom:10px" />
    <textarea id="import-text" placeholder="…ou colle ici le JSON d'un tri partagé" style="height:200px"></textarea>
    <p class="hint" id="import-err" style="color:#ff6b6b"></p>
  </div>
  <div class="dlg-foot">
    <button id="import-close">Annuler</button>
    <button class="btn-primary" id="import-load">Charger ce tri</button>
  </div>
</dialog>
...
// ---- Export / share (the full tri; the btn-export-state click handler is defined below) ----
$('#share-close').onclick = () => $('#export-state-dlg').close();
$('#share-copy').onclick = async () => { await navigator.clipboard.writeText($('#share-text').value); $('#share-copy').textContent = 'Copié ✓'; setTimeout(() => $('#share-copy').textContent = 'Copier', 1500); };
$('#share-dl').onclick = () => {
  const blob = new Blob([$('#share-text').value], { type: 'application/json' });
  const u = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = u; a.download = 'yallah-tri-partage.json'; a.click();
  URL.revokeObjectURL(u);
};

// ---- Import a shared state ----
$('#btn-import').onclick = () => { $('#import-text').value = ''; $('#import-err').textContent = ''; $('#import-file').value = ''; $('#import-dlg').showModal(); };
$('#import-close').onclick = () => $('#import-dlg').close();
...
function applyPresentation(pres) {
  if (!pres) return;
  if (pres.sort && $('#cat-sort')) $('#cat-sort').value = pres.sort;
  if (typeof pres.search === 'string') $('#search').value = pres.search;
  if (typeof pres.onlyUndecided === 'boolean') $('#only-undecided').checked = pres.onlyUndecided;
}
$('#import-load').onclick = () => {
  let obj;
  try { obj = JSON.parse($('#import-text').value); } catch { $('#import-err').textContent = 'JSON invalide.'; return; }
  // Accept new format ({ activites: { state }, presentation }) or a raw state ({ groups }).
  const st = obj && obj.activites && obj.activites.state ? obj.activites.state : obj;
  if (!st || !Array.isArray(st.groups)) { $('#import-err').textContent = "Ce fichier n'est pas un tri valide (clé « groups » manquante)."; return; }
  pushUndo();                       // back up current state before replacing
  state = hydrate(st);
  applyPresentation(obj && obj.presentation);
  save();
  $('#import-dlg').close();
  render();
};
...
// Strip page-only fields so exported objects match the activities.json schema.
function cleanActivity(a) {
  const { photo, userAdded, ...rest } = a;
  return rest;
}
// The final result (kept activities + vote remap) — the part useful for the real DB work.
function buildDecision() {
  const drop = computeDrops();
  const dropped = new Set(drop.map(d => d.id));
  const allIds = [...DATA.activities.map(a => a.id), ...state.userActivities.map(a => a.id)];
  const kept = allIds.filter(id => !dropped.has(id)).map(id => {
    const out = cleanActivity(BY[id]);
    const g = findGroupOf(id);
    if (g && g.id.indexOf('single-') !== 0) out.regroupement = g.name; // real categorizations only
    return out;
  });
  const remap = {};
  drop.forEach(d => { remap[d.id] = d.reason === 'merged' ? d.mergedInto : null; });
  return {
    summary: { source: DATA.generatedFrom, added: state.userActivities.length, removed: drop.length, kept: kept.length },
    activities: kept,
    remap,
  };
}
// The transient, web-only view state (sort, filters) — not used for the final work.
function getPresentation() {
  return {
    sort: ($('#cat-sort') && $('#cat-sort').value) || 'count',
    search: $('#search').value,
    onlyUndecided: $('#only-undecided').checked,
  };
}
// One file, two parts: "activites" (data + result, for the final work) and
// "presentation" (transient site view, just so a recipient sees the same thing).
function buildShare() {
  return { version: 1, activites: { state, final: buildDecision() }, presentation: getPresentation() };
}
// Multi-activity categories still undecided (and not fully discarded) = not yet treated.
function untreatedCategories() {
  return state.groups.filter(g => g.ids.length > 1 && !g.decision && g.ids.some(id => !isDiscarded(id)));
}
$('#btn-export-state').onclick = () => {
  $('#share-text').value = JSON.stringify(buildShare(), null, 2);
  const un = untreatedCategories();
  const el = $('#export-alert');
  if (un.length) {
    const nb = un.reduce((s, g) => s + g.ids.filter(id => !isDiscarded(id)).length, 0);
    el.className = 'alert warn';
    el.innerHTML = '⚠ <b>' + un.length + ' catégorie' + (un.length > 1 ? 's' : '') + '</b> pas encore tranchée'
      + (un.length > 1 ? 's' : '') + ' (ni « Fusionner » ni « Garder séparé »), soit <b>' + nb + ' activité'
      + (nb > 1 ? 's' : '') + '</b> :<br><span class="alist">' + un.map(g => esc(g.name)).join(' · ') + '</span>';
  } else {
    el.className = 'alert ok';
    el.innerHTML = '✓ Toutes les catégories à plusieurs activités ont été tranchées.';
  }
  $('#export-state-dlg').showModal();
};
...
const out = join(root, 'activites-doublons.html');
writeFileSync(out, html);
console.log(`Wrote ${out}`);

// --- Web variant served at /admin/activities (Vercel) ---
// Same page, but: photo paths absolute (/photos/…) instead of repo-relative, and
// an embedded seed state (scripts/admin-seed.json) so a fresh visitor lands on the
// pre-built triage. The page's strict-CSP-incompatible inline script is allowed by
// a relaxed CSP scoped to /admin/* in vercel.json.
let webHtml = html.replace(/public\/photos\//g, '/photos/');
